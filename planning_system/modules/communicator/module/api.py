...
def send_to_encryption(details):
    if not details:
        abort(400)
...
@app.route('/task', methods=['POST'])  #Разрешаем только POST запросы, так как передаем данные
def get_task():
    try:
        data = request.get_json()  # Получаем JSON из тела запроса
        if data is None:
            abort(400, description="Missing JSON in request body")
        coordinates = data.get('coordinates')
        if coordinates is None:
            abort(400, description="Missing coordinates in request body")

        # Теперь у вас есть:
        # - name: значение из URL (например, 'task' или 'car123')
        # - coordinates: значение из JSON-объекта в теле POST-запроса

        print(f"Received access request for name: {MODULE_NAME}, coordinates: {coordinates}")

        # Здесь ваш код обработки запроса (например, вызов send_to_encryption)
        details_to_send = {"operation": "access", "coordinates": coordinates, "name": MODULE_NAME}
        send_to_encryption(details_to_send)

        return jsonify({"status": "ok", "message": f"Access granted for {MODULE_NAME}"})
